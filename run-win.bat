@echo off
rem
rem Rolly303 -- Windows installer.
rem
...
rem   3. installs the standalone app to   %ProgramFiles%\Rolly303
...
set "HERE=%~dp0"
set "STANDALONE=%HERE%build\Rolly303_artefacts\Release\Standalone\Rolly303.exe"
set "VST3SRC=%HERE%build\Rolly303_artefacts\Release\VST3\Rolly303.vst3"
...
copy /y "%STANDALONE%" "%INSTDIR%\Rolly303.exe" >nul
if errorlevel 1 (
...
robocopy "%VST3SRC%" "%VST3DIR%\Rolly303.vst3" /e /njh /njs /ndl /nfl /nc /ns >nul
if errorlevel 8 (
...
powershell -NoProfile -Command "$ws = New-Object -ComObject WScript.Shell; foreach ($p in '%STARTMENU%\%SHORTCUT%', '%Public%\Desktop\%SHORTCUT%') { $s = $ws.CreateShortcut($p); $s.TargetPath = '%INSTDIR%\Rolly303.exe'; $s.WorkingDirectory = '%INSTDIR%'; $s.Save() }"

rem ---- 5) Register the uninstaller in "Add or remove programs".
echo Registering uninstaller...
reg add "%REGKEY%" /v DisplayName     /t REG_SZ /d "Rolly303" /f >nul
reg add "%REGKEY%" /v DisplayVersion  /t REG_SZ /d "1.0.0" /f >nul
reg add "%REGKEY%" /v Publisher       /t REG_SZ /d "Rolly303" /f >nul
reg add "%REGKEY%" /v InstallLocation /t REG_SZ /d "%INSTDIR%" /f >nul
reg add "%REGKEY%" /v DisplayIcon     /t REG_SZ /d "%INSTDIR%\Rolly303.exe" /f >nul
...
echo.
echo Installed:
echo   Standalone : %INSTDIR%\Rolly303.exe
echo   VST3       : %VST3DIR%\Rolly303.vst3
echo   Shortcuts  : Start Menu and Desktop
echo   Uninstall  : %INSTDIR%\uninstall.bat  (also in "Add or remove programs")
echo.
choice /c YN /m "Launch Rolly303 now"
if not errorlevel 2 start "" "%INSTDIR%\Rolly303.exe"
endlocal
exit /b 0
